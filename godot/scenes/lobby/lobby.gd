...
onready var lobby_selection_row = preload("res://scenes/lobby/lobby_list_button.tscn")

# -> Godot functions <-


func _ready() -> void:
	steam_username.text = SteamHelper.STEAM_USERNAME
...
func _join_Lobby(lobbyID: int) -> void:
	...
	SteamHelper.LOBBY_MEMBERS.clear()
	Steam.joinLobby(lobbyID)


func _get_Lobby_Members() -> void:
	SteamHelper.LOBBY_MEMBERS.clear()
	var MEMBERS: int = Steam.getNumLobbyMembers(SteamHelper.LOBBY_ID)

	for MEMBER in range(0, MEMBERS):
		var MEMBER_STEAM_ID: int = Steam.getLobbyMemberByIndex(SteamHelper.LOBBY_ID, MEMBER)
		var MEMBER_STEAM_NAME: String = Steam.getFriendPersonaName(MEMBER_STEAM_ID)
		_add_Player_List(MEMBER_STEAM_ID, MEMBER_STEAM_NAME)


func _add_Player_List(steam_id: int, steam_name: String):
	SteamHelper.LOBBY_MEMBERS.append({"steam_id": steam_id, "steam_name": steam_name})

	player_list.clear()
	var tmp: int = 0

	for MEMBER in SteamHelper.LOBBY_MEMBERS:
...
func _send_Chat_Message() -> void:
	...
	var SENT: bool = Steam.sendLobbyChatMsg(SteamHelper.LOBBY_ID, MESSAGE)
...
func _leave_Lobby() -> void:
	if SteamHelper.LOBBY_ID != 0:
		...
		Steam.leaveLobby(SteamHelper.LOBBY_ID)

		SteamHelper.LOBBY_ID = 0

		get_lobby_name.text = "Lobby Name"
		player_count.text = "Players (0)"
		player_list.clear()

		for MEMBERS in SteamHelper.LOBBY_MEMBERS:
...
func _start_game() -> void:
	if SteamHelper.LOBBY_MEMBERS == []:
		_display_message("You haven't joined/made a lobby yet.")
		return

	SteamHelper.send_P2P_Packet("all", {"message": "start_game"})
...
func _on_Lobby_Joined(lobbyID: int, _permissions: int, _locked: bool, _response: int) -> void:
	...
	SteamHelper.make_P2P_Handshake()
...
func _on_Lobby_Chat_Update(
	_lobbyID: int, _changedID: int, makingChangeID: int, chatState: int
) -> void:
...
func _on_Lobby_Match_List(lobbies: Array) -> void:
	for LOBBY in lobbies:
		...
		var LOBBY_BUTTON = lobby_selection_row.instance()
...
func _check_Command_Line() -> void:
	...
	if ARGUMENTS.size() > 0:
		for ARGUMENT in ARGUMENTS:
			...
			if SteamHelper.LOBBY_INVITE_ARG:
				_join_Lobby(int(ARGUMENT))

			if ARGUMENT == "+connect_lobby":
				SteamHelper.LOBBY_INVITE_ARG = true
